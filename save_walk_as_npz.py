...
def generate_random_walks_tensor(pc_tensor, num_walks=32, seq_len=256, k_neighbors=8):
    """
    Differentiable random walk generator that returns walk coordinate tensors.
    
    Args:
        pc_tensor: (N, 3) torch tensor of point cloud coordinates
        Returns: (num_walks, seq_len, 3) walk tensor of 3D coordinates
    """
    N = pc_tensor.shape[0]
    dists = torch.cdist(pc_tensor, pc_tensor)  # (N, N)
    knn_indices = torch.topk(dists, k=k_neighbors + 1, largest=False).indices[:, 1:]  # skip self

    walks_idx = torch.zeros((num_walks, seq_len), dtype=torch.long, device=pc_tensor.device)
    ...
    for w in range(num_walks):
        ...
        walks_idx[w] = torch.tensor(walk, dtype=torch.long, device=pc_tensor.device)

    walks = pc_tensor[walks_idx]  # shape: (num_walks, seq_len, 3)
    return walks
