...
scanobjectnn_labels = [
    'bag', 'bin', 'box', 'cabinet', 'chair', 'desk', 'display', 'door', 'shelf', 'table', 'bed', 'pillow', 'sink',
    'sofa', 'toilet'
]
...
def prepare_kdtree(point_cloud):
    ...
    assert point_cloud['kdtree_query'].shape[1] == (n_nbrs - 1), 'Number of kdtree_query is wrong: ' + str(
        point_cloud['kdtree_query'].shape[1])
...
def prepare_modelnet40_normal_resampled():
    ...
    for part in ['test', 'train']:
        ...
        for npoints in npoints_list:
            ...
            datapaths = [(files_name[i], shape_names[i], os.path.join(p, shape_names[i], files_name[i]) + '.txt') for i
                         in range(len(files_name))]
            for f in datapaths:
                ...
                if os.path.isfile(out_fn + '.npz'):  # npz = numpy file type
                    continue
