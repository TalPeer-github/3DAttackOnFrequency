...
def get_mesh():
    ...
    model_fn = os.path.expanduser(
        '~') + '/datasets_processed/human_benchmark_sig_17/sig17_seg_benchmark/meshes/test/shrec/10.off'
    mesh = load_mesh(model_fn)
    mesh, _, _ = remesh(mesh, 4000)
    mesh = EasyDict({'vertices': np.asarray(mesh.vertices), 'faces': np.asarray(mesh.triangles),
                     'n_faces_orig': np.asarray(mesh.triangles).shape[0]})
