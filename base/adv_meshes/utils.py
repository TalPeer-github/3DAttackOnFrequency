...
def merge_some_models_for_visualization(models):
    ...
    for model in models[1:]:
        ...
        this_vertices[:, 0] += x_shift - this_vertices[:, 0].min() + (
                    this_vertices[:, 0].max() - this_vertices[:, 0].min()) * 0.1
        all_vertices = np.vstack((all_vertices, this_vertices))
        x_shift = all_vertices.max(axis=0)[0]
    return all_vertices, all_faces


def visualize_model(vertices_, faces_, title=' ', vertex_colors_idx=None, cpos=None, v_size=None, off_screen=False,
                    walk=None, opacity=1.0,
                    all_colors='white', face_colors=None, show_vertices=True, cmap=None, edge_colors=None,
                    edge_color_a='white', dual_object=None,
                    dual_object_shift=0.7, window_size=[1024, 768], line_width=1, show_edges=True,
                    edge_colors_list=None, point_size=15, show_surface=True):
    ...
    for pos in range(n_obj2show):
        ...
        surf = pv.PolyData(vertices, faces)
        if show_surface:
            p.add_mesh(surf, show_edges=show_edges, edge_color=edge_color_a, color=all_colors, opacity=opacity,
                       smooth_shading=True,
        ...
        if show_vertices and vertex_colors_idx is not None:
            ...
            for c in np.unique(vertex_colors_idx):
                if c != -1:
                    ...
                    if v_size is None:
                        p.add_mesh(pv.PolyData(surf.points[idxs]), color=colors[c], point_size=point_size,
            ...
            if edge_colors is None:
                ...
                for this_e_color in range(np.max(edge_colors) + 1):
                    ...
                    if np.any(this_edges):
                        ...
                        walk_mesh = pv.PolyData(vertices, walk_edges)
                        p.add_mesh(walk_mesh, show_edges=True, edge_color=colors[this_e_color],
    ...
    if keep:
        ...
        if params.full_accuracy_test is not None:
            if params.network_task == 'semantic_segmentation':
                ...
                accuracy, _ = evaluate_clustering.calc_accuracy_test(params=params, dnn_model=dnn_model,
                                                                     verbose_level=0,
                                                                     **params.full_accuracy_test)
            ...
            with open(params.logdir + '/log.txt', 'at') as f:
                f.write('Accuracy: ' + str(np.round(np.array(accuracy) * 100, 2)) + '%, Iter: ' + str(
                    iterations.numpy()) + '\n')
