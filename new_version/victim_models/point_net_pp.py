...
def train():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_idx in progress_bar:
            ...
            if batch_idx % 50 == 0:
                p = prediction.max(1)[1].eq(data.y).sum().item() / len(data.y)
                print(f"Batch {batch_idx}: Train loss = {loss.item()} | Train corrects = {p:.2f}")
        ...
        print("===========================================================================")
        print(
            f"Training Epoch {epoch} / {num_epochs}:\n Train loss = {train_epoch_loss} | Train accuracy = {train_epoch_accuracy:.2f}")
        print("===========================================================================")
        ################################
        # val_step(epoch)
        ################################
        ...
        print("===========================================================================")
        print(
            f"Validation Epoch {epoch} / {num_epochs}:\n Val loss = {val_epoch_loss} | Val accuracy = {val_epoch_accuracy:.2f}")
        print("===========================================================================")
        ...
        if save_checkpoint:
            torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                       ...
                       f"checkpoints/epochs{epoch}_{int(val_epoch_accuracy * 100)}val_acc.pt")
