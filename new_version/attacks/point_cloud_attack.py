...
from utils.config import modelnet10_classes,args
from utils.attack_utils import ClipPointsLinf, UntargetedLogitsAdvLoss, L2Dist, CrossEntropyAdvLoss,ClipPointsL2


def visualize_point_cloud_spectral(pos_orig,pos_adv, true_class="",pred_class="",adv_class="",title_orig="",title_adversarial="",spectral=True,linf=True):
    ...
    fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(15, 12), subplot_kw={'projection': '3d'},) #height_ratios=[0.6,0.4])

    axes[0,0].scatter(pos_orig[:, 0], pos_orig[:, 1], pos_orig[:, 2],edgecolor='#670D2F', s=50, alpha=0.5,c='#EF88AD',linewidth=1.25)
    axes[0,0].set_title(f"Clean Prediction: {pred_class}")

    axes[0,1].scatter(pos_adv[:, 0], pos_adv[:, 1], pos_adv[:, 2],edgecolor='#670D2F', s=50, alpha=0.5,c='#EF88AD',linewidth=1.25)
...
def plot_attack(orig_pos, adv_pos,clean_pred,adv_pred,true_label,linf=False):
    ...
    visualize_point_cloud_spectral(pos_orig=orig_pos, pos_adv=adv_pos, true_class=true_class,pred_class=pred_class,adv_class=adv_class,
                                title_orig=title_orig,title_adversarial=title_adversarial_pred,spectral=True,linf=linf)
...
def get_Laplace_from_pc(ori_pc,data):
    ...
    with torch.no_grad():
        idx = knn(pc, 30)
        # idx = torch.tensor(torch.cat(torch.unbind(data.walks_idx,dim=0),dim=0),dtype=torch.long)
        pc = pc.transpose(2, 1).contiguous()  #(B, N, 3)
        #print(f"idx.shape: {idx.shape}")
        # print(f"pc.T shape: {pc.shape}")
        # print(f"pc.unsqueeze(1).shape: {pc.unsqueeze(1).shape}")
        # print(f"pc.unsqueeze(2).shape: {pc.unsqueeze(2).shape}")
        point_mat = pc.unsqueeze(2) - pc.unsqueeze(1)  #(B, N, N, 3)
        A = torch.exp(-torch.sum(point_mat.square(), dim=3))  #(B, N, N)
        mask = torch.zeros_like(A)
        # Reshape idx to match mask dimensions
        B, N, k = idx.shape
        idx = idx.reshape(B, N, k, 1)  # Add extra dimension for scatter
        mask = mask.unsqueeze(-1)  # Add extra dimension to mask
...
class CWAOF:
    def __init__(self, model, device='cuda', eps=0.2, steps=10, lr=1e-3, 
                 clip_mode='linf', budget = 0.18,low_pass=100,gamma=0.5,max_iter=150):
        ...
        self.adv_func = UntargetedLogitsAdvLoss(kappa=30.)
    ...
    def attack(self, data):
        ...
        # print(f"unsqueeze_pos.shape: {unsqueeze_pos.shape}")
        ...
        best_dist = 1e10 # np.array([1e10] * B)
        best_attack = None # ori_data.transpose(1, 2).squeeze(0).clone().detach()
        ...
        for step in tqdm(range(self.steps)):
            ...
            for iteration in range(self.max_iter):
                ...
                # pred_current_val = adv_pred.detach().cpu().numpy() 
                # lfc_pred_current_val = lfc_pred.detach().cpu().numpy() 
                # input_val = adv_data.detach().cpu().numpy() 
                # label_val_list = np.array([label_val]) # if B == 1

                # for pc_batch_idx, tup in enumerate(zip(dist_current_val, pred_current_val, lfc_pred_current_val, label_val_list, input_val)):
                #     dist, attack_pred, attack_lfc_pred, true_label, current_adv_pc = tup

                attack_success_cond = attack_successes_count > 0 # adv_pred != target and lfc_pred != target
                dist_cond = dist_current_val.item(0) < best_dist
                #final_cond = (step == self.steps - 1)
                if (dist_cond and attack_success_cond):# or step == self.steps - 1:
                    best_dist = dist_current_val.item(0)
                    best_attack = adv_data.detach().clone()
            #torch.cuda.empty_cache()
        
        
        # best_attack = best_attack[0] if B == 1 else best_attack

        if best_attack is None:
            best_attack = adv_data.detach().clone()
        # print(f"best_attack.shape: {best_attack.shape}")
        ...
        # adv_data = Data(pos = adv_pc, x = data.x, batch = data.batch, y=data.y)
        ...
        for data in tqdm(dataloader, desc="Spectral Adversarial Attack progress"):
            ...
            try:
                if i % 5 == 0:
                    ...
                    adv_acc = ((clean_preds != adversarial_preds) * (true_labels == clean_preds)).sum().item() / len(clean_preds)
                    print(f"{i} Samples:")
